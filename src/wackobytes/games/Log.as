...
public class Log {
    ...
    static private function log(s:*, type:String = null, param:* = null):void {
        ...
        if (type == "Dump") {
            ...
            if (logTextField_) logTextField_.text += (dump_(serializedObject, true, param.maxDepth) + "\n");
            if (outputTextField_) outputTextField_.text +=(dump_(serializedObject, true, param.maxDepth) + "\n");
        } else {
            if (type) logString = '[' + type + '] ' + logString;
            if (prefix) logString = '[' + prefix + '] ' + logString;
            trace(logString);
            if (outputTextField_) outputTextField_.text += (logString + "\n");
...
//              }
       ...
       // if (outputTextField_) outputTextField_ = outputTextField_.maxScrollV;
    ...
    static public function showLogWindow(parent:*):void {
        if (!logWindow_) {
            ...
            logTextField_ = new TextField(parent.width, 50, "this is the log", "Arial", 15);
            var tf:TextFormat = new TextFormat();
            tf.font = "Arial";
            tf.size = 10;

            closeLogWindowButton_= new TextField(30, 30, "close log");
